    ...
    def get_games(self):
        ...
        if shortcuts:
            ...
            for shortcut in shortcuts:
                ...
                game_key, game_dict = self.read_shortcut(
                    shortcut[:-4], os.path.join(self.shortcuts_path, shortcut))
                ...
                if self.shortcut_banners:
                    for banner in self.shortcut_banners:
                        if banner.lower().startswith(a_name.lower()):
                            game_dict[
                                GameKeys.BANNER_PATH] = "Shortcuts\\" + banner
                            break
                if not game_dict.get(GameKeys.BANNER_PATH):
                    game_dict[
                        GameKeys.BANNER_PATH] = "Shortcuts\\%s.jpg" % a_name
                print("\t\tPath: '%s'" % path)
                return (
                    a_name,
                    game_dict, )
